...
public class SongListController {
    ...
    public Object allSongList() {
        return new SuccessMessage<List<SongList>>("所有歌单", songListService.allSongList()).getMessage();
    }

    // 获取推荐歌单
    @RequestMapping(value = "/songListRecommend", method = RequestMethod.GET)
    public Object recommendSongList(HttpServletRequest req, @RequestParam(value = "userId",defaultValue = "") String userId){
        ...
        System.out.println("-------------推荐用户id:" + userId2);
        //如果userId为空或该用户收藏为0就返回所有歌单
        if(userId2.equals("null") || collectService.collectionOfUser(Integer.parseInt(userId2)).size()==0) {
            return new SuccessMessage<List<SongList>>("所有歌单", songListService.allSongList()).getMessage();
        }else{
            return new SuccessMessage<List<SongList>>("用户"+userId2+"的推荐歌单", recommendSongListService.recommendSongListByCollect(Integer.parseInt(userId2))).getMessage();
        }
    }
    ...
    public Object songListOfLikeTitle(HttpServletRequest req) {
        String title = req.getParameter("title").trim();
        return new SuccessMessage<List<SongList>>(title, songListService.likeTitle('%' + title + '%')).getMessage();
    }
    ...
    public Object songListOfStyle(HttpServletRequest req) {
        String style = req.getParameter("style").trim();
        return new SuccessMessage<List<SongList>>("风格为"+style+"的歌单", songListService.likeStyle('%' + style + '%')).getMessage();
    }
